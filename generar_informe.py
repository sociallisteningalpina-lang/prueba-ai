...
def load_campaign_info() -> dict:
    """Loads campaign_info.json from config directory."""
    config_path = CONFIG_DIR / "campaign_info.json"
    try:
        with open(config_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
            return data.get("campaign_info", {})
    except Exception:
        return {}
...
def analyze_comments_with_gemini(df_comments: pd.DataFrame, campaign_info: dict) -> tuple:
    """Pass 2: Classifies sentiment and assigns comments to discovered topics."""
    ...
    print(f"🤖 Pass 2: Classifying {total_comments} comments with Gemini in batches of {batch_size}...")
...
INSTRUCTIONS:
1. 'sentiment': Classify strictly as 'Positivo', 'Negativo', or 'Neutro'. Account for sarcasm, emojis, and regional slang.
...
def run_report_generation():
    """Reads Excel data, analyzes sentiment/topics with Gemini, and generates index.html."""
    ...
    campaign_info = load_campaign_info()

    # Call Gemini for Classification
    df_comments['sentimiento'], df_comments['tema'] = analyze_comments_with_gemini(df_comments, campaign_info)
            ...
            document.addEventListener('DOMContentLoaded', () => {{
                ...
                Object.assign(charts, {{
                    postCount: new Chart(document.getElementById('postCountChart'), {{ 
                        ...
                        options: {{ responsive: true, maintainAspectRatio: false, plugins: {{ title: {{ display: true, text: 'Distribución de Pautas por Red Social' }}, legend: {{ display: true, position: 'bottom' }} }} }},
                        plugins: [doughnutLabelPlugin]
                    }}),
                    sentiment: new Chart(document.getElementById('sentimentChart'), {{ 
                        type: 'doughnut',
                        data: {{ labels: [], datasets: [{{}}] }},
                        options: {{ responsive: true, maintainAspectRatio: false, plugins: {{ title: {{ display: true, text: 'Distribución de Sentimientos' }}, legend: {{ display: true, position: 'bottom' }} }} }},
                        plugins: [doughnutLabelPlugin]
                    }}),
                    topics: new Chart(document.getElementById('topicsChart'), {{ 
                        type: 'doughnut',
                        data: {{ labels: [], datasets: [{{}}] }},
                        options: {{ responsive: true, maintainAspectRatio: false, plugins: {{ title: {{ display: true, text: 'Distribución por Temas' }}, legend: {{ display: true, position: 'bottom' }} }} }},
                        plugins: [doughnutLabelPlugin]
                    }}),
                    ...
                    hourly: new Chart(document.getElementById('hourlyChart'), {{ type: 'bar', options: {{ responsive: true, maintainAspectRatio: false, scales: {{ x: {{ stacked: true }}, y: {{ stacked: true, position: 'left' }}, y1: {{ position: 'right', grid: {{ drawOnChartArea: false }} }} }}, plugins: {{ title: {{ display: true, text: 'Volumen de Comentarios por Hora' }} }} }} }})
                }});
                ...
                const updatePostLinks = () => {{
                    ...
                    let tableHTML = '<table><tr><th>Pauta</th><th>Comentarios</th><th>Enlace</th></tr>';
